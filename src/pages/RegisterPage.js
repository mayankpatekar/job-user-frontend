...
const RegisterPage = () => {
    ...
    const loginForm = async () => {
        try {
            ...
            const response = await fetch('http://localhost:5003/user/register', {
                method: 'POST',
                headers: {
    ...
    <main>
            <div className='bg-slate-200 h-screen grid place-content-center'>
                <form onSubmit={handleSubmit} className="max-w-md bg-white p-4 rounded-sm">
                    ...
                    <h1 className='font-bold text-3xl pb-3 pt-3'>Register</h1>
                    ...
                    <div className='pt-2 pb-2 w-full text-left '><Link className='text-blue-700 hover:text-blue-950' to="/login">
                        Already have account ?
                    </Link>
                    </div>
                    <div>
                        <button type='submit' className="bg-sky-500 w-full text-white p-2 rounded-lg hover:bg-sky-950">Register</button>
                    </div>
